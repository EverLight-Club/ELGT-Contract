...
library Rules {
    ...
    function setRule(Rule storage rule, uint256 _intervalFreezeBlock,/* uint256 _firstPercent,*/ uint256 _percent, uint256 _maxAmount) internal {
        ...
        //rule.firstPercent = _firstPercent;
    }
    ...
    function freezeAmount(Rule storage rule, uint256 baseAmount, uint256 startFrozenBlock, uint256 lastFreezeBlock, uint256 currentBlock) internal view returns(uint256) {
        ...
        return baseAmount.mul(rule.percent).mul(factor).div(10000);
    }
}
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _totalSupply_add(uint256 amount) internal virtual {
        _totalSupply += amount;
    }

    function _totalSupply_sub(uint256 amount) internal virtual {
        _totalSupply -= amount;
    }

    function _balance_add(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: balance add to the zero address");
        _balances[account] += amount;
    }

    function _balance_sub(address account, uint256 amount) internal virtual {
        require(account != address(0), "ERC20: balance sub to the zero address");
        _balances[account] -= amount;
    }
}
...
contract ELGTToken is AccountFrozenBalances, ERC20, ERC20Burnable, Ownable, ERC20Permit, ERC20Votes {
    ...
    function queryStartBlock() public view returns (uint256) {
        return freezeStartBlock;
    }
    ...
    function mintBatchFrozenTokensForRole(address[] memory accounts, uint256[] memory amounts, RoleType _role) public onlyOwner onlyReady canBatchMint(amounts)  roleCanBatchMint(uint256(_role), amounts) returns (bool) {
